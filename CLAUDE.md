...
Cuoco is a three-phase Context Driven Development workflow: setup, recipe, cook. Code is never written before a reviewed plan exists. All artifacts live in `.artifacts/`, a hidden directory tracked normally on every branch.
...
Commits follow Conventional Commits (https://www.conventionalcommits.org/en/v1.0.0/). Format: type(scope): subject. Types: feat, fix, test, refactor, docs, chore. Each commit is atomic — one logical change. Do not append Co-Authored-By lines to commit messages.
...
One-time project initialisation. Creates the `.artifacts/` directory and generates foundational project documents.
...
1. Create the `.artifacts/` directory and its subdirectories (`feat/`, `code-style/`).
2. Interactive Q&A — ask the user batched questions about the project:
   - What does the project do? Who is it for?
   - What are the core features / capabilities?
   - What does success look like? Key constraints?
3. Generate `.artifacts/product.md` from the answers.
4. Tech stack — offer the user a choice: infer the tech stack from the existing codebase (inspect package.json, requirements.txt, go.mod, Cargo.toml, file extensions, etc.) or specify it manually. Either way, generate `.artifacts/tech-stack.md`.
...
6. Copy the fixed code style guides into `.artifacts/code-style/`.
7. Extract a preliminary feature list from `product.md`. For each feature determine dependencies (which features must complete first). Write `.artifacts/feat/index.json`.
8. Present the feature list and dependency graph to the user for review. Adjust based on feedback.
9. Commit all generated artifacts with message: chore(cuoco): initial project setup.

### Output

`.artifacts/product.md`, `.artifacts/tech-stack.md`, `.artifacts/feat/index.json`, `.artifacts/code-style/*.md`, and optionally `.artifacts/product-guidelines.md`. All committed on the current branch.
...
9. Commit all artifacts with message: docs(cuoco): add recipe for <feature-id>.
...
`.artifacts/feat/<feature-id>/research.md` and `plan.md`. Feature status "planned" in `index.json`. Feature branch `feat/<feature-id>` created (identical to main — no code changes). All committed on the feature branch.
...
6. For each step in `plan.md`, in order:
   a. Mark the step [IN PROGRESS] in `plan.md`. Commit.
   ...
   e. Mark the step [DONE] in `plan.md`. Commit.
...
7. When all steps are [DONE], set the feature status to "done" in `index.json`. Commit.

...
<agents>
  <about>
    Cuoco ("cook" in Italian) is a Context Driven Development workflow for Claude Code. It structures the SDLC into three phases — setup, recipe (research + planning), and cook (implementation) — so that code is never written before a reviewed plan exists. All workflow artifacts live on an isolated Git orphan branch (cuoco/artifacts) mounted at artifacts/ via a Git worktree. This keeps the code history clean while preserving a full audit trail of decisions and findings.
  </about>

  <principles>
    <principle>No code before plan. Never write or modify source code until a plan exists and the user has approved it.</principle>
    <principle>Artifacts are the source of truth. Every decision, finding, and plan is persisted to a file in artifacts/. Chat is ephemeral; artifacts survive context compaction and session boundaries.</principle>
    <principle>Branch-driven development. Every feature lives on its own branch (feat/&lt;id&gt;). No direct commits to main.</principle>
    <principle>KISS. Start with the simplest implementation that satisfies current requirements. Complexity needs a demonstrated, concrete justification.</principle>
  </principles>
  ...
  <workflow>
    ...
    <command name="cuoco:setup">
      <purpose>One-time project initialisation. Creates the artifact infrastructure and generates foundational project documents.</purpose>

      <preconditions>A Git repository must exist (or be initialised as part of setup).</preconditions>
    ...
    <command name="cuoco:f-recipe">
      <purpose>Research and plan a single feature. Produces research.md and plan.md. No source code is created or modified — only files in artifacts/.</purpose>
    ...
    <command name="cuoco:f-cook">
      <purpose>Implement a planned feature by executing its plan.md.</purpose>
  ...
  <artifacts>
    ...
    <file name="feat/index.json">
      ...
      A feature can enter /cuoco:f-recipe when status is "pending" and all depends_on features have status "done".
      A feature can enter /cuoco:f-cook when status is "planned".
    </file>

    <file name="feat/&lt;id&gt;/research.md">
